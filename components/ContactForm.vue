    ...
    <form @submit.prevent="onSubmit" ref="contactForm">
      <div>
        <input v-model="contactData.name" :class="{
          ...
          type="text" id="name" name="name" placeholder="Bitte geben Sie ihren Namen ein..." autocomplete="off" required
      ...
      <div>
        <input v-model="contactData.email" :class="{
          ...
          placeholder="Bitte geben Sie ihre Emailadresse ein" autocomplete="off" required @blur="emailTouched = true" />
      ...
      <div>
        ...
        }" id="message" name="message" cols="30" rows="10"
          placeholder="Nachricht hinterlassen..." required @blur="messageTouched = true"></textarea>
      ...
      <div class="checkbox">
        ...
        <span>
          Ich habe die <NuxtLink to="/privacypolicy" activeClass="link-active">Datenschutzerklärung</NuxtLink> gelesen
          und stimme der Verarbeitung meiner
          Daten zu.
        </span>
      </div>
...
const prop = defineProps({
  ...
  borderPrimary: {
    type: String,
    default: 'var(--secondary-brand-color)',
  }
});

const cssProps = computed(() => {
  return {
    '--active': prop.borderActive,
    '--hover': prop.borderHover,
    '--border-primary-color': prop.borderPrimary
  }
})

onMounted(() => {
  /**
   * Applies CSS properties from the `cssProps` reactive object to the root element (`<html>`).
   * Iterates over each key-value pair in `cssProps.value` and sets the corresponding CSS variable
   * on the document's root element using `document.documentElement.style.setProperty`.
   *
   * @param {Object} cssProps - A reactive object containing CSS properties and their values.
   */
  Object.entries(cssProps.value).forEach(([key, value]) => {
...
form {
  ...
  >div {
    ...
    textarea {
      ...
      border: 0.125rem solid var(--border-primary-color);
      border-radius: 10px;
      padding: 15px 25px;
